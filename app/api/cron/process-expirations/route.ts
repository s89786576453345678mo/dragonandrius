...
// Funcao para criar cliente Supabase
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log("[EXPIRATION] Starting expiration check at", now.toISOString())

    // Buscar todos os pagamentos de PLANOS aprovados
    // (nao order bumps, upsells, downsells - apenas planos que sao assinaturas)
    const { data: planPayments, error: paymentsError } = await supabase
      ...
      `)
      .eq("status", "approved")
      .in("product_type", ["plan", "subscription", "plano"])
      .not("duration_days", "is", null)
      .gt("duration_days", 0) // Ignorar vitalicios (0 dias)

    if (paymentsError) {
      console.error("[EXPIRATION] Error fetching payments:", paymentsError)
      return NextResponse.json({ error: paymentsError.message }, { status: 500 })
    }

    if (!planPayments || planPayments.length === 0) {
      console.log("[EXPIRATION] No plan payments to check")
    ...
    console.log(`[EXPIRATION] Checking ${planPayments.length} plan payments`)
    ...
    console.log(`[EXPIRATION] Found ${Object.keys(userLatestPayment).length} unique user/bot combinations`)
    ...
    for (const [key, payment] of Object.entries(userLatestPayment)) {
      try {
        ...
        console.log(`[EXPIRATION] User ${payment.telegram_user_id} expired ${daysSinceExpire} days ago`)

        // Verificar se ja foi processado (marcado como expired)
        ...
        if (!bot?.token) {
          console.log(`[EXPIRATION] Bot ${payment.bot_id} not found`)
          continue
        }
        ...
        // Banir de grupos VIP (se configurado)
        if (flowConfig?.subscription?.kickFromGroup !== false && flowConfig?.deliverables) {
          for (const deliverable of flowConfig.deliverables) {
            if (deliverable.type === "vip_group" && 
              ...
              console.log(`[EXPIRATION] Banning user ${payment.telegram_user_id} from group ${deliverable.vipGroupChatId}`)
              ...
              if (banResult.ok) {
                console.log(`[EXPIRATION] Successfully banned user from group`)
              } else {
                console.log(`[EXPIRATION] Failed to ban: ${banResult.description}`)
              }
            }
          }
        }
        ...
        await supabase.from("subscription_expirations").insert({
          ...
          console.log("[EXPIRATION] Could not insert expiration record:", err.message)
        })

        usersExpired++

      } catch (userError) {
        console.error(`[EXPIRATION] Error processing ${key}:`, userError)
        errors++
      }
    }

    console.log(`[EXPIRATION] Completed. Expired: ${usersExpired}, Notifications: ${notificationsSent}, Errors: ${errors}`)
